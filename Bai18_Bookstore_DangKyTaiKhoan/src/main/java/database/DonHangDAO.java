...
public class DonHangDAO implements DAOInterfare<DonHang>{
	...
	public ArrayList<DonHang> selectAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				KhachHang khachHang = new KhacHangDAO()
						.selectById(new KhachHang(maKH, trangThai, sql, maDH, maKH, diaChiNguoiMua, diaChiNhanHang, hinhThucThanhToan, null, 0, trangThai, false));
				DonHang dh = new DonHang(maDH, khachHang, diaChiNguoiMua, diaChiNhanHang, trangThai, hinhThucThanhToan, trangThai, soTienThanhToan, soTienDaThanhToan, soTienConThieu, null, null);
	...
	public int update(DonHang t) {
		...
		String sql = "UPDATE donhang" + " SET " + "ma_khachh_hang=?" + ", dia_chi_mua_hang=?" + ",dia_chi_nhan_hang=?"+ ",hinh_thuc_thanh_toan=?"
				+ ",trang_thai_thanh_toan=?" + ",so_tien_thanh_toan=?" + ",so_tien_da_thanh_toan=?" + ",so_tien_con_thieu=?" + ",ngay_dat_hang=?"
