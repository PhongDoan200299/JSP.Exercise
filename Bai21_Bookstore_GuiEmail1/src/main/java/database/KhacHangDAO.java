...
public class KhacHangDAO implements DAOInterfare<KhachHang> {
	...
	public KhachHang selectById(KhachHang t) {
		...
		try {
			...
			while (rs.next()) {
				...
				String email = rs.getString("email");
				boolean dangKyNhanBangTin = rs.getBoolean("dang_ky_nhan_bang_tin");
				String maXacThuc = rs.getString("ma_xac_thuc");
				Date thoiGianHieuLucCuaMaXacThuc = rs.getDate("thoi_gian_hieu_luc_cua_ma");
				Boolean trangThaiXacThuc = rs.getBoolean("trang_thai_xac_thuc");

				ketQua = new KhachHang(maKhacHang, tenDangNhap, matKhau, hoVaTen, gioiTinh, diaChi, diaChiNhanHang, diaChiMuaHang, ngaySinh, soDienThoai, email, dangKyNhanBangTin, maXacThuc, thoiGianHieuLucCuaMaXacThuc, dangKyNhanBangTin);
	...
	public int updateVerifyInformation(KhachHang t) {
		...
		try {
			...
			// Bước 2: tạo ra đối tượng statement
			String sql = "UPDATE khachhang " + " SET " + " ma_xac_thuc=?" + ", thoi_gian_hieu_luc_cua_ma_xac_thuc=?" + ", trang_thai_xac_thuc=?"
					+ " WHERE ma_khach_hang=?";
	...
	public int changePassword(KhachHang t) {
		int ketQua = 0;
