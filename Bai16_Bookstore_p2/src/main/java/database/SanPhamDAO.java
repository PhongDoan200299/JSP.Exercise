...
public class SanPhamDAO implements DAOInterfare<SanPham>{
	...
	public ArrayList<SanPham> selectAll() {
		...
		try {
			...
			while(rs.next()) {
				...
				SanPham sanPham = new SanPham(maSanPham, tenSanPham, tacGia, namXuatBan, giaNhap, giaGoc, giaBan, soLuong, theLoai, moTa, ngonNgu);
				ketQua.add(sanPham);
			}
	...
	public SanPham selectById(SanPham t) {
		...
		try {
			...
			while (rs.next()) {
				...
				TacGia tacGia = (new TacGiaDAO().selectById(new TacGia(maTacGia, "", null, "")));
				TheLoai theLoai = (new TheLoaiDAO().selectById(new TheLoai(maTheLoai, "")));
				ketQua = new SanPham(maSanPham, tenSanPham, tacGia, namXuatBan, giaNhap, giaGoc, giaBan, soLuong, theLoai, moTa, ngonNgu);
				break;
	...
	public int insertAll(ArrayList<SanPham> arr) {
	    ...
	    try {
	        ...
	        // Bước 3: Thực hiện lặp qua danh sách sản phẩm và thêm các tham số vào câu lệnh SQL
