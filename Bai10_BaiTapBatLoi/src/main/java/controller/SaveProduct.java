...
public class SaveProduct extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(maSanPham == null || maSanPham.trim().length()==0) {
			...
			e_maSanPham+= "Mã sản phẩm 123 đã tổn tại, vui lòng nhập lại MSP!";		
		}
		
		String e_gia = "";
		double giaBan = Double.parseDouble(giaBan_String);
		double giaNhap = Double.parseDouble(giaNhap_String);
		if(giaBan < giaNhap) {
			e_gia = "Vui lòng nhập lại giá bán và giá nhập";
		}
		...
		request.setAttribute("value_giaBan_String",giaBan_String);
		request.setAttribute("value_giaNhap_String",giaNhap_String);
		request.setAttribute("value_hanSuDung_String",hanSuDung_String);
		request.setAttribute("value_vat_String",vat_String);
